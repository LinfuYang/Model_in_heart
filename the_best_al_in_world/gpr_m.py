...
a_gpr = A_GPR()
gp_l = a_gpr.creat_gpr_model(x_train_l, y_train_l)
joblib.dump(gp_l, 'gp_l')
size_gp_l = sys.getsizeof(gp_l)
print('size_gp_l:%s bit' %size_gp_l)
...
# 假设网络带宽：4800kb/s， 传输距离2000KM， 电信号传播速度光速的2/3   = 20Wkm/s

# 计算训练集、测试集数据的大小
# size_train_data = sys.getsizeof(x_train_l) + sys.getsizeof(y_train_l)
...
print('低有效数据构建的模型传输到边缘节点的延迟：', time_FS(data_len=size_gp_l, speed_fs=4800e+3)*1000)
