...
class A_GPR:
    ...
    def creat_gpr_model(self, x_data, y_data):
        ...
        m, n = np.shape(x_data)
        k_RBF = GPy.kern.RBF(input_dim=n, variance=0.5, lengthscale=0.6)
        # k_RBF = GPy.kern.Exponential(input_dim=n, variance=0.3)
    ...
    def predict_mu_var(self, x_new, model_gp, re_var=True):
        ...
        if re_var:
            ...
            if x_new[0, 0] >=65 and x_new[0, 0] <=75:
                y_mean_lf += 5
    ...
    def creat_gp_model(self, max_loop=15, x_init_l=None, y_init_l=None, x_init_h=None, y_init_h=None, x_conda=None, y_conda=None, list_iter=None, data_test=None):
        ...
        for it in range(3, max_loop+1):
            ...
            if it in list_iter:
                y_temp_pre = []
                # 0
                y_temp_pre.append(self.h_f_pre(data_test[0], lf_gp, hf_gp, w_lf, w_hf))
                # print(data_test[0])
                # 1
                ...
                for i in range(3, np.shape(data_test)[0]):
                    ...
                    if d_NSM[i] - d_NSM[i - 3] == 3 or abs(d_week[i] - d_week[i-3]) == 1:
                        ...
                        x_test_i[4] = 0
                    # if d_NSM[i] >= 65 and d_NSM[i] <= 75:
                    #     y_temp_pre.append(self.h_f_pre(x_test_i, lf_gp, hf_gp, w_lf, w_hf) + 10)
                    # else:
